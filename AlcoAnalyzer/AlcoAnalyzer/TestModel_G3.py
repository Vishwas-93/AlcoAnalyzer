...
column_g2 = df['G2'].values.reshape(-1,1)
#
...
X = np.concatenate((sex_values,column_age,column_medu, mj1,mj2,mj3,mj4,mj5,f1,f2,f3,f4,f5, r1,r2,r3,r4,column_study_time,
                    schoolsup_data_values,fams_values, higher_data_values, column_goout, column_dalc, column_walc, column_g1, column_g2), axis=1)
...
def predictGradeThree(data):
    ...
    try:
        X_test_new = np.concatenate((sex_values, age, medu, mj1, mj2, mj3, mj4, mj5, f1, f2, f3, f4, f5, r1, r2, r3, r4,
                                     study_time, schoolsup_data_values, fams_values,higher_data_values, goout, dalc, walc, g1, g2), axis=1)
        # print(X_test_new.shape)
        X_test_new=X_norm.transform(X_test_new)
        filename = 'g3model.sav'
        g3model = pickle.load(open(filename, 'rb'))
        # val = np.reshape(X_test_new[1, 24], (1, -1))
        # print(val.shape)
        y_pred = g3model.predict(X_test_new)[0]
        # print(y_pred)
        return y_pred
...
# g3model = pickle.load(open(filename, 'rb'))
# val =np.reshape(X_test[6,:],(1,-1))
# print(val.shape)
# y_pred=g3model.predict(val)[0]
# print(y_pred)
# print(y_test[6])
